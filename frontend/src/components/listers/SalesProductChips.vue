<template>
    <div style="margin: 0 -15px 0 -15px;">
        <v-list two-line v-if="value.length > 0">
            <v-list-group v-model="isExpansion">
                <template v-slot:activator>
                    <v-list-item-content>
                        <v-list-item-title>SalesProduct</v-list-item-title>
                    </v-list-item-content>
                </template>
        ...
        <SalesProduct v-if="editMode && tick" offline :isNew="true" :inList="true"
...
<script>
    import SalesProduct from '../SalesProduct.vue';

    export default {
        name: 'SalesProductManager',
        components: {
            SalesProduct
        },
        props: {
